...
ggplot(data = target_means, aes(x = relative_target, y = mean_rts)) +
  ...
  labs(caption = "An ANOVA test shows a difference in RT depending on target location, F(1.74,34.86)=12.925, p<.001.")
...
```
```{r ANOVA tests for accuracy}

res.aov <- anova_test(data = pilot_dist, dv = accuracy, wid = ID, within = relative_hp_dist)
get_anova_table(res.aov)

res.aov <- anova_test(data = pilot_targ, dv = accuracy, wid = ID, within = relative_target)
get_anova_table(res.aov)
