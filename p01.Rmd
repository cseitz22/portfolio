...
trimmed <- pilot[pilot$acc==100, ] %>% group_by(ID, relative_hp_dist) %>%
           ...
           select(ID, relative_hp_dist, RT, block_num) %>%
           as.data.frame()
...
ggplot(data = sum_stats, aes(x = relative_hp_dist, y = mean_rts)) +
  ...
  labs(caption = "An ANOVA test shows a difference in RT depending on distractor location, F(4,80)=33.3, p<.001.")
...
Assumption of sphericity is automatically checked during the ANOVA test. Next, we'll do the ANOVA test. The ANOVA shows a significant difference among distractor locations, F(4,80)=32.621, p<.001. The two different methods used to run the ANOVA gave me the same results (YAY)!
...
Below, I added the average rt for the low probability locations and the difference between the low probability and high probability locations ignoring na values. 
